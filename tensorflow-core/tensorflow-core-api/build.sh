...
if [[ -d $BAZEL_VC ]]; then
    ...
    export BUILD_FLAGS="--copt=-msse4.1 --copt=-msse4.2 --copt=-mavx `#--copt=-mavx2 --copt=-mfma` --cxxopt=-std=c++14 --host_cxxopt=-std=c++14 --linkopt=-lstdc++ --host_linkopt=-lstdc++ --copt=-D_GLIBCXX_USE_CXX11_ABI=0 --cxxopt=-g"
